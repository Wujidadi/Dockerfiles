...
* `docker buildx` commands: (`$D` is a custom alias of the log directory)
  ```sh
  docker buildx build --no-cache --progress=plain --push --platform linux/amd64,linux/arm64 --rm -t wujidadi/ap:3.5.1 -t wujidadi/ap:latest ap/3.5.1 > /dev/null 2>&1 | tee $D/docker-build-ap.log
  ```
* OS: **Ubuntu 22.04 LTS**
