...
## Apache2 with PHP
docker buildx build --no-cache --progress=plain --push --platform linux/amd64,linux/arm64 --rm -t wujidadi/ap:3.5 -t wujidadi/ap:latest -t wujidadi/ap:latest ap/3.5 2>&1 | tee $D/docker-build-ap.log
docker run -d -p 50000:80 -it --name Test wujidadi/ap:3.5
...
docker run -d -p 50000:80 -it --name Test wujidadi/nginx-php:2.5
# docker exec -it Test zsh
docker exec -it Test vim --version
docker exec -it Test nano -V
...
docker run -d -p 50000:80 -it --name Test wujidadi/ubuntu-tuned:2.5
# docker exec -it Test zsh
docker exec -it Test vim --version
docker exec -it Test nano -V
docker stop Test; docker rm Test
